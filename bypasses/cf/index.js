...
function defaults (params) {
  ...
  let defaultParams = (!isRequestModule && this.defaultParams) || {
    ...
    cloudflareMaxTimeout: 5001,
    // followAllRedirects - follow non-GET HTTP 3xx responses as redirects
    followAllRedirects: true,
    // Support only this max challenges in row. If CF returns more, throw an error
    challengesToSolve: 3000,
    ...
    agentOptions: {
      ...
      ciphers: crypto.constants.defaultCipherList + 'AES256-SHA'
    }
  };
...
function onRequestResponse (options, error, response, body) {
  ...
  response.isCloudflare = /^(cloudflare|am6-min01|openresty)/i.test('' + headers.server);
  ...
  if (response.isCloudflare && response.isHTML) {
    ...
    onCloudflareResponse2(options, response, body);
  } else {
...
function onCloudflareResponse (options, response, body) {
  ...
  const isRedirectChallenge = stringBody.indexOf('strict-origin-when-cross-origin') !== -1 ||
    stringBody.indexOf('strict-origin-when-cross-origin') !== -1;


      const isRedirectChallenge2 = stringBody.indexOf('Checking your browser..') !== -1 ||
    stringBody.indexOf('/nooder-cgi/recaptcha/check.js') !== -1;
...
function onCloudflareResponse2 (options, response, body) {
  const callback = options.callback;
  ...
  try {
    ...
    if (error instanceof CaptchaError && typeof options.onCaptcha2 === 'function') {
      // Give users a chance to solve the reCAPTCHA via services such as anti-captcha.com
      return onCaptcha2(options, response, stringBody);
    }

    return callback(error);
  }

  const isChallenge2 = stringBody.indexOf('a = document.getElementById(\'jschl-answer\');') !== -1;
  const isChallenge3 = stringBody.indexOf('a = document.getElementById(\'jschl_answer\');') !== -1;
  ...
  const isRedirectChallenge = stringBody.indexOf('strict-origin-when-cross-origin') !== -1 ||
    stringBody.indexOf('strict-origin-when-cross-origin') !== -1;
      ...
      const isRedirectChallenge2 = stringBody.indexOf('Verifying your browser, please wait...') !== -1 ||
    stringBody.indexOf('bf-form') !== -1;
...
if (isRedirectChallenge2) {
  ...
  if (response.statusCode === 503) {
    return onChallenge(options, response, stringBody);
    return onChallenge2(options, response, stringBody);
  }
...
function detectRecaptchaVersion (body) {
  ...
  if (body.indexOf('challenge_headline') !== -1 || /__cf_chl_captcha_tk__=(.*)/i.test(body)) { // Test for ver2 first, as it also has ver2 fields
    //} else if (body.indexOf('challenge_headline') !== -1 || /__cf_chl_captcha_tk__=(.*)/i.test(body)) {
    return 'ver2';
     
    // Old version < Dec 2019
  } else if (body.indexOf('challenge_headline') !== -1 || /__cf_chl_jschl_tk__=(.*)/i.test(body)) {
    return 'ver1';
  }
...
function onChallenge (options, response, body) {
  ...
  const payload = { jschl_vc,blazing_answer, pass,vc,r, jschl_answer };
  ...
  match = body.match(/type="hidden" id="jschl-vc" name="jschl_vc" value="(\w+)"/);
  if (!match) {
  ...
  match = body.match(/type="hidden" id="bf-answer" name="blazing_answer" value="(\w+)"/);
  if (!match) {
    cause = 'challengeId (blazing_answer) extraction failed';
    return callback(new ParserError(cause, options, response));
  }

  payload.blazing_answer = match[1];
  ...
  match = body.match(/type="hidden" name="vc" value="(.+?)"/);
  if (!match) {
    cause = 'Attribute (vc) value extraction failed';
    return callback(new ParserError(cause, options, response));
  }
  ...
  match = body.match(/type="hidden" name="r" value="(.+?)"/);
  if (!match) {
    cause = 'Attribute (r) value extraction failed';
    return callback(new ParserError(cause, options, response));
  }
  ...
  match = body.match(/class="challenge-form" id="challenge-form" action="(.+?)" method="(.+?)" enctype="(.+?)"/);
  if (match && match[2] && match[2] === 'POST') {
    ...
    options.uri = uri.protocol + '//' + uri.host + '/cdn-cgi/challenge-platform/orchestrate/jsch/v1';
...
function onCaptcha (options, response, body) {
  ...
  const payload = { /*g-re-captcha-response*/ };
  ...
  if (isRecaptchaVer2) {
    ...
    //(.+?)
    match = body.match(/id="challenge-form" action="(.+?)" method="POST"/);
    if (!match) {
  ...
  const submit = function (error) {
    ...
    onSubmitCaptcha2(options, response);
  };
...
//onCaptcha
function onCaptcha2 (options, response, body) {
  const recaptchaVer = detectRecaptchaVersion(body);
  const isRecaptchaVer2 = recaptchaVer === 'ver1';
  const callback = options.callback;
  // UDF that has the responsibility of returning control back to cloudscraper
  const handler = options.onCaptcha2;
  // The form data to send back to Cloudflare
...
function onSubmitCaptcha (options, response) {
  ...
  const isRecaptchaVer1 = response.captcha.version === 'ver2';

  if (!response.captcha.form['h-captcha-response']) {
  ...
  if (isRecaptchaVer2) {
    ...
    options.uri = uri.protocol + '//' + uri.host + '?__cf_chl_captcha_tk__';
  }
...
///////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////
function onSubmitCaptcha2 (options, response) {
  const callback = options.callback;
  const uri = response.request.uri;
  const isRecaptchaVer2 = response.captcha.version === 'ver2';
  const isRecaptchaVer1 = response.captcha.version === 'ver2';
  ...
  if (isRecaptchaVer1) {
    options.qs = {
      __cf_chl_jschl_tk__: response.captcha.formActionUri.match(/?__cf_chl_jschl_tk__=(.*)/)[1]
    };
  ...
  if (isRecaptchaVer1) {
    ...
    options.uri = uri.protocol + '//' + uri.host + '?__cf_chl_jschl_tk__';
  }
